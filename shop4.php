...
详细填写收货人信息、送货方式、书目信息等，确认无误后点击“提交订单”。
	...
	<div class="navitem"><a  class="active" href="shop4.php" >分店4</a></div>
...
<div class="hor" style="background-color: #FEFDED"><strong>亚运村店(分店4)</strong></div>
<div class="row">
	<div class="left">
	  <ul class="leftnav">
...
echo  ' <div style="height:5%"><form name="form6">
	...
	<img src="shop4.jpg" height=auto width=100%></a>
	</div>
	<div class="right" style="background-color: #FEFDED">
	 
		<div class="right" style="height:40%;"><font color="#F40E12"><strong>本月活动：</strong><br>
		<center>双十二购书狂欢节</center>
		<strong>活动细则：</strong><br>
		&#12288;&#12288;2019.12.9~2019.12.15期间，凡在本店购书（不分种类），2本以上享受八五折优惠，5本以上享受八折优惠。</font>
		</div>
		<div class="right" style="height:54%">
		<strong>本店特色：</strong><br>
		&#12288;&#12288;亚运村店2017年建成，是“神奇书店”最年轻的分店，也是占地面积最大的书店。店内装修明亮，适合阅读。书店24小时运营，店内有咖啡店供您休息小憩。<br>
		<strong>地址：</strong><br>北京市朝阳区慧忠北里309号亚运村图书大厦3层<br>
		<strong>联系电话：</strong>66351880</div>
...
/*分店4热销书*/
$result=mysqli_query($conn,"SELECT title,book.ISBN,publisher, price, classification, allauthor, alltranslator,url FROM book, transaction,storage WHERE (book.ISBN=transaction.ISBN and book.ISBN=storage.ISBN and storage.shopno='4') GROUP BY book.ISBN ORDER BY SUM(number)DESC");
$i=0;
...
$result=mysqli_query($conn,"SELECT title, publisher, price, edition,classification, allauthor, alltranslator,url,Cintroduction,Eintroduction FROM book, `book-classification`,`storage` WHERE book.ISBN=`book-classification`.ISBN AND book.ISBN=`storage`.ISBN AND `storage`.shopno=4 AND `book-classification`.cno=4");
...
$result=mysqli_query($conn,"SELECT title, publisher, price, edition,classification, allauthor, alltranslator,url,Cintroduction,Eintroduction FROM book, `book-classification`,`storage` WHERE book.ISBN=`book-classification`.ISBN AND book.ISBN=`storage`.ISBN AND `storage`.shopno=4 AND `book-classification`.cno=1");
while($row=mysqli_fetch_array($result))
{
    echo'<div class="book1">';
    echo'<div class="image1"><a target="_blank" href="_blank"><img src="'.$row['url'].'" height="150"></a></div>';
...
$result=mysqli_query($conn,"SELECT title, publisher, price, edition,classification, allauthor, alltranslator,url,Cintroduction,Eintroduction FROM book, `book-classification`,`storage` WHERE book.ISBN=`book-classification`.ISBN AND book.ISBN=`storage`.ISBN AND `storage`.shopno=4 AND `book-classification`.cno=7");
while($row=mysqli_fetch_array($result))
{
    echo'<div class="book1">';
    echo'<div class="image1"><a target="_blank" href="_blank"><img src="'.$row['url'].'" height="150"></a></div>';
...
$result=mysqli_query($conn,"SELECT title, publisher, price, edition,classification, allauthor, alltranslator,url,Cintroduction,Eintroduction FROM book, `book-classification`,`storage` WHERE book.ISBN=`book-classification`.ISBN AND book.ISBN=`storage`.ISBN AND `storage`.shopno=4 AND `book-classification`.cno=2");
while($row=mysqli_fetch_array($result))
{
    echo'<div class="book1">';
    echo'<div class="image1"><a target="_blank" href="_blank"><img src="'.$row['url'].'" height="150"></a></div>';
...
$result=mysqli_query($conn,"SELECT title, publisher, price, edition,classification, allauthor, alltranslator,url,Cintroduction,Eintroduction FROM book, `book-classification`,`storage` WHERE book.ISBN=`book-classification`.ISBN AND book.ISBN=`storage`.ISBN AND `storage`.shopno=4 AND `book-classification`.cno=3");
while($row=mysqli_fetch_array($result))
{
    echo'<div class="book1">';
    echo'<div class="image1"><a target="_blank" href="_blank"><img src="'.$row['url'].'" height="150"></a></div>';
...
$result=mysqli_query($conn,"SELECT title, publisher, price, edition,classification, allauthor, alltranslator,url,Cintroduction,Eintroduction FROM book, `book-classification`,`storage` WHERE book.ISBN=`book-classification`.ISBN AND book.ISBN=`storage`.ISBN AND `storage`.shopno=4 AND `book-classification`.cno=5");
while($row=mysqli_fetch_array($result))
{
    echo'<div class="book1">';
    echo'<div class="image1"><a target="_blank" href="_blank"><img src="'.$row['url'].'" height="150"></a></div>';
...
$result=mysqli_query($conn,"SELECT title, publisher, price, edition,classification, allauthor, alltranslator,url,Cintroduction,Eintroduction FROM book, `book-classification`,`storage` WHERE book.ISBN=`book-classification`.ISBN AND book.ISBN=`storage`.ISBN AND `storage`.shopno=4 AND `book-classification`.cno=6");
while($row=mysqli_fetch_array($result))
{
    echo'<div class="book1">';
    echo'<div class="image1"><a target="_blank" href="_blank"><img src="'.$row['url'].'" height="150"></a></div>';
...
$result=mysqli_query($conn,"SELECT title, publisher, price, edition,classification, allauthor, alltranslator,url,Cintroduction,Eintroduction FROM book, `book-classification`,`storage` WHERE book.ISBN=`book-classification`.ISBN AND book.ISBN=`storage`.ISBN AND `storage`.shopno=4 AND `book-classification`.cno=8");
while($row=mysqli_fetch_array($result))
{
    echo'<div class="book1">';
    echo'<div class="image1"><a target="_blank" href="_blank"><img src="'.$row['url'].'" height="150"></a></div>';
...
<div class="row">
  <div class="navitem" style="flex:50%;background-image:url('bg.jpg');">
	<div class="navitem">神奇书店在哪里 分店4</div></div>
