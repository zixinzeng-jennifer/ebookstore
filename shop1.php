...
详细填写收货人信息、送货方式、书目信息等，确认无误后点击“提交订单”。
    ...
    <div class="navitem" ><a  class="active" href="shop1.php" >分店1</a></div>
...
<div class="hor" style="background-color: #FEFDED"><strong>北四环西路店(分店1)</strong></div>
<div class="row">
	<div class="left">
	  <ul class="leftnav">
...
echo  ' <div style="height:5%"><form name="form6">
	...
	<img src="shop1.jpg" height=auto width=100%></a>
	</div>
	<div class="right" style="background-color: #FEFDED">
		...
		&#12288;&#12288;2019.12.1~2019.12.31，凡在本店购买文学类书籍，在总店利润的基础上再打八五折</font>
		</div>
		<div class="right" style="height:54%">
		<strong>本店特色：</strong><br>
		&#12288;&#12288;“神奇书店”资历最深分店，在迁址并且装修以后，新增大量环形公共阅读区域，是与朋友相约读书、购书的不错选择。人文类书籍为本店主打，因此经常会有人文类书籍的促销活动，同时每月9日会举办小型文学交流会。<br>
		<strong>地址：</strong>北四环西路68号<br>
		<strong>联系电话：</strong>64801180</div>
...
/*分店1本月活动*/
echo'<div class="hor" style="align-content:center;height:auto">本月活动</div>
<div class="row">';
$result=mysqli_query($conn,"SELECT title, book.ISBN,publisher, price, classification, allauthor, alltranslator,url FROM storage,book where storage.ISBN=book.ISBN and `storage`.shopno='1'and `book`.classification='人文' GROUP BY book.ISBN ");
$i=0;
while($i<8){
	...
	echo"现价："."$".round(($row['price']*0.9*0.85),2)."<br>";
	echo'</font>';
...
/*分店1热销书*/
$result=mysqli_query($conn,"SELECT title,book.ISBN,publisher, price, classification, allauthor, alltranslator,url FROM book, transaction,storage WHERE (book.ISBN=transaction.ISBN and book.ISBN=storage.ISBN and storage.shopno='1') GROUP BY book.ISBN ORDER BY SUM(number)DESC");
$i=0;
...
$result=mysqli_query($conn,"SELECT title, publisher, price, edition,classification, allauthor, alltranslator,url,Cintroduction,Eintroduction FROM book, `book-classification`,`storage` WHERE book.ISBN=`book-classification`.ISBN AND book.ISBN=`storage`.ISBN AND `storage`.shopno=1 AND `book-classification`.cno=4");
...
$result=mysqli_query($conn,"SELECT title, publisher, price, edition,classification, allauthor, alltranslator,url,Cintroduction,Eintroduction FROM book, `book-classification`,`storage` WHERE book.ISBN=`book-classification`.ISBN AND book.ISBN=`storage`.ISBN AND `storage`.shopno=1 AND `book-classification`.cno=1");
while($row=mysqli_fetch_array($result))
{
    echo'<div class="book1">';
    echo'<div class="image1"><a target="_blank" href="_blank"><img src="'.$row['url'].'" height="150"></a></div>';
...
$result=mysqli_query($conn,"SELECT title, publisher, price, edition,classification, allauthor, alltranslator,url,Cintroduction,Eintroduction FROM book, `book-classification`,`storage` WHERE book.ISBN=`book-classification`.ISBN AND book.ISBN=`storage`.ISBN AND `storage`.shopno=1 AND `book-classification`.cno=7");
while($row=mysqli_fetch_array($result))
{
    echo'<div class="book1">';
    echo'<div class="image1"><a target="_blank" href="_blank"><img src="'.$row['url'].'" height="150"></a></div>';
...
$result=mysqli_query($conn,"SELECT title, publisher, price, edition,classification, allauthor, alltranslator,url,Cintroduction,Eintroduction FROM book, `book-classification`,`storage` WHERE book.ISBN=`book-classification`.ISBN AND book.ISBN=`storage`.ISBN AND `storage`.shopno=1 AND `book-classification`.cno=2");
while($row=mysqli_fetch_array($result))
{
    echo'<div class="book1">';
    echo'<div class="image1"><a target="_blank" href="_blank"><img src="'.$row['url'].'" height="150"></a></div>';
...
$result=mysqli_query($conn,"SELECT title, publisher, price, edition,classification, allauthor, alltranslator,url,Cintroduction,Eintroduction FROM book, `book-classification`,`storage` WHERE book.ISBN=`book-classification`.ISBN AND book.ISBN=`storage`.ISBN AND `storage`.shopno=1 AND `book-classification`.cno=3");
while($row=mysqli_fetch_array($result))
{
    echo'<div class="book1">';
    echo'<div class="image1"><a target="_blank" href="_blank"><img src="'.$row['url'].'" height="150"></a></div>';
...
$result=mysqli_query($conn,"SELECT title, publisher, price, edition,classification, allauthor, alltranslator,url,Cintroduction,Eintroduction FROM book, `book-classification`,`storage` WHERE book.ISBN=`book-classification`.ISBN AND book.ISBN=`storage`.ISBN AND `storage`.shopno=1 AND `book-classification`.cno=5");
while($row=mysqli_fetch_array($result))
{
    echo'<div class="book1">';
    echo'<div class="image1"><a target="_blank" href="_blank"><img src="'.$row['url'].'" height="150"></a></div>';
...
$result=mysqli_query($conn,"SELECT title, publisher, price, edition,classification, allauthor, alltranslator,url,Cintroduction,Eintroduction FROM book, `book-classification`,`storage` WHERE book.ISBN=`book-classification`.ISBN AND book.ISBN=`storage`.ISBN AND `storage`.shopno=1 AND `book-classification`.cno=6");
while($row=mysqli_fetch_array($result))
{
    echo'<div class="book1">';
    echo'<div class="image1"><a target="_blank" href="_blank"><img src="'.$row['url'].'" height="150"></a></div>';
...
$result=mysqli_query($conn,"SELECT title, publisher, price, edition,classification, allauthor, alltranslator,url,Cintroduction,Eintroduction FROM book, `book-classification`,`storage` WHERE book.ISBN=`book-classification`.ISBN AND book.ISBN=`storage`.ISBN AND `storage`.shopno=1 AND `book-classification`.cno=8");
while($row=mysqli_fetch_array($result))
{
    echo'<div class="book1">';
    echo'<div class="image1"><a target="_blank" href="_blank"><img src="'.$row['url'].'" height="150"></a></div>';
...
<div class="row">
  <div class="navitem" style="flex:50%;background-image:url('bg.jpg');">
	<div class="navitem">神奇书店在哪里 分店1</div></div>
