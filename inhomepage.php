...
function personalsale()
{
	...
	window.location.href="personalsale.php";
	}
function personalcollection()
{
	document.form1.action="personalcollection.php";
	document.form1.submit();
	window.location.href="personalcollection.php";
	}
function get(num)
{
...
function srcrec()
{
	...
	function searchpost()
	{
		...
		//alert(s);
		var url="search.php?type=2&bookprice=all&bookshop=all&search="+s;
		form6.action=url;
		form6.method="post";
		form6.submit();
	}
...
<div class="row" style="background-image:url('bg.jpg');">
	...
	<div class="navitem" ><a  class="active" href="inhomepage.php" >首页</a></div>
	<div class="navitem"><a href = "javascript:void(0)" onclick = "document.getElementById('light').style.display='block';document.getElementById('fade').style.display='block'" >会员优惠</a></div>
...
<div id="fade" class="black_overlay"></div> 
	...
	<div class="navitem"><a href = "javascript:void(0)" onclick = "document.getElementById('light2').style.display='block';document.getElementById('fade2').style.display='block'" >使用说明</a></div>
...
echo '	<div class="main">';
echo  ' <div style="height:auto"><form method="post" name="form6">
				<input id="search" name="src" list="history" placeholder="输入书名、作者、译者、简介..." onclick=srcrec()>';
$sql="SELECT search FROM uhistory WHERE userno='".$_COOKIE['uno']."'GROUP BY search ORDER BY date DESC";
$result=mysqli_query($conn,$sql);
$i=0;
echo'<datalist id="history" style="display:none;">';
while($i<5 && $row=mysqli_fetch_array($result))
{
 echo' <option value="'.$row['search'].'">';
 $i+=1;
}
$sql="SELECT * from user WHERE userno='".$_COOKIE['uno']."'";
$result=mysqli_query($conn,$sql);
$row=mysqli_fetch_array($result);
echo'</datalist>';
echo'          <input type="button"  id="searchicon" value="搜索" onclick="searchpost()">
				<a class="comsearch" href="_blank">高级检索</a></form> </div>
	<img src="0.png" height=auto width=100%></a>
	</div>
	<div class="right"><div class="image">
			<a target="_blank" href="_blank"><img src="'.$row['img'].'" height="150px"></a>
            <br>
            <br>
            <p><a href="_blank"><img src="mb.jpg" height="20px" width="20px"></a>  '.$_COOKIE['uname'].', 欢迎！</p>
		</div>
		<div>
			<form method="post" action="_blank" name="form1">
				<br>
				<br>
				<input type="button" value="查看我的购买记录"  onclick=personalsale()>
				<br>
				<br>
                <input type="button" value="查看我的收藏记录"  onclick=personalcollection()>
				<br>
				<br>
...
echo'<div class="hor" >猜您喜欢</div><div class="row">';
...
{
    while($row=mysqli_fetch_array($result))//取浏览记录中的书
    {
    $sql2="SELECT * FROM simbook WHERE ISBN1='".$row['ISBN']."'";
    $result2=mysqli_query($conn,$sql2);
    while($row2=mysqli_fetch_array($result2))
        {
        $sql1="SELECT * FROM search WHERE ISBN='".$row2['ISBN2']."'";
        $result1=mysqli_query($conn,$sql1);
        $row1=mysqli_fetch_array($result1);
        $num=$row1['result']+$row2['sim'];
        $sql3="UPDATE search SET result=".$num."WHERE ISBN='".$row2['ISBN2']."'";
        mysqli_query($conn,$sql3);
        }
    }
    $sql4="SELECT * FROM `order`,transaction WHERE `order`.sno=transaction.sno AND userno='".$_COOKIE['uno']."'";
    $result4=mysqli_query($conn,$sql4);
    while($row4=mysqli_fetch_array($result4))
    {
     $sql5="UPDATE search SET result=0 WHERE ISBN='".$row4['ISBN']."'";
...
{   
    ...
    echo "《".$row['title'] ."》 " . "  "."版本:".$row['edition']."<br>";
    echo "作者:".$row['allauthor']. "  译者:".$row['alltranslator'] . "<br>";
    echo "出版社:".$row['publisher']." "."分类:".$row['classification']."<br>";
    echo "$".($row['price']);
    echo '</div>';}
    else {
        $result=mysqli_query($conn,"SELECT book.ISBN,title, edition, publisher, price, classification, allauthor, alltranslator,url FROM book, transaction WHERE book.ISBN=transaction.ISBN GROUP BY book.ISBN ORDER BY SUM(number)ASC");
        $row=mysqli_fetch_array($result);
        echo'<div class="book"><div class="image"><a  href="specific_book.php?ISBN='.$row['ISBN'].'"><img src="'.$row['url'].'" height="150"></a></div>';
        echo "《".$row['title'] ."》 " . "  "."版本:".$row['edition']."<br>";
        echo "作者:".$row['allauthor']. "  译者:".$row['alltranslator'] . "<br>";
...
$month=date("m");
$day=date("d");
$result=mysqli_query($conn,"select book.title,book.publisher, book.price,book.edition, book.classification, book.allauthor, book.alltranslator,book.url,`book-orderdate`.`ISBN`,sum(`book-orderdate`.`number`),`book-orderdate`.`month`,`book-orderdate`.`day` from `book-orderdate`,`book` where (((`book-orderdate`.`month`={$month}-1) and (`book-orderdate`.`day`>={$day})) or((`book-orderdate`.`month`={$month}) and (`book-orderdate`.`day`<={$day}))) and book.ISBN=`book-orderdate`.`ISBN` group by `book-orderdate`.`ISBN` ORDER BY SUM(`book-orderdate`.`number`)DESC");
$i=0;
echo'<div class="hor" style="width:100%;">热销</div><div class="row" style="width:100%;">';
while($i<4){
    $row=mysqli_fetch_array($result);
    echo'<div class="book"><div class="image"><a  href="specific_book.php?ISBN='.$row['ISBN'].'"><img src="'.$row['url'].'" height="150"></a></div>';
    echo "《".$row['title'] ."》 " . "  "."版本:".$row['edition']."<br>";
...
<div  style="float:left; text-align:right; width:35%;"><p>使用说明</p><br/><a href = "#totop" onclick = "document.getElementById('light2').style.display='block';document.getElementById('fade2').style.display='block'">使用流程</a><br/><a href = "#totop" onclick = "document.getElementById('light').style.display='block';document.getElementById('fade').style.display='block'">会员优惠</a></div>
<div  style="float:left; text-align:center; width:30%;"><p>分类导航</p><br/>&nbsp;<a href=#c4 onclick=" return get(4);">&nbsp;&nbsp;教材</a> &nbsp;&nbsp;&nbsp;    <a href=#c1 onclick=" return get(1);">计算机</a><br/><a href=#c7 onclick=" return get(7);">&nbsp;&nbsp;理工      </a>  &nbsp;&nbsp;&nbsp;   <a href=#c2 onclick=" return get(2);">人文&nbsp;&nbsp;</a><br/><a href=#c3 onclick=" return get(3);">&nbsp;&nbsp;社科     </a>   &nbsp;&nbsp;&nbsp;  <a href=#c5 onclick=" return get(5);">经管&nbsp;&nbsp;</a><br/><a href=#c6 onclick=" return get(6);">&nbsp;&nbsp;医学     </a>&nbsp;&nbsp;&nbsp;<a href=#c8 onclick=" return get(8);">     其他&nbsp;&nbsp;</a></div>
<div  style="float:left; text-align:left; width:35%;"><p>分店导航</p><br/><a href="shop1.php">分店1</a><br/><a href="shop2.php">分店2</a><br/><a href="shop3.php">分店3</a><br/><a href="shop4.php">分店4</a><br/></div>
</div>
</body>
</html>